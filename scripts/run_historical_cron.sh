#!/usr/bin/env bash
# run_historical_cron.sh — Phase 20: operativa automàtica backfill históric
#
# Modes:
...
case "$MODE" in
    daily)
        ...
        $RUN_CMD \
            ...
            --to "$YESTERDAY" \
            --sleep "$SLEEP"
        echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] daily DONE"
        ;;

    retry-failed)
        # Reintenta tots els mesos fallats
        echo "  → retry-failed per $SYMBOL"
        # Necessitem rang ampli per cobrir qualsevol mes fallat
        $RUN_CMD \
            --symbol "$SYMBOL" \
            --from "2003-01-01" \
            --to "$(date -u '+%Y-%m-%d')" \
            --retry-failed \
            --sleep "$SLEEP"
        echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] retry-failed DONE"
        ;;
    ...
    gap-repair)
        ...
        $RUN_CMD \
            ...
            --no-skip-existing \
            --sleep "$SLEEP"
        echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] gap-repair DONE"
        ;;

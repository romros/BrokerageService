...
- ✅ **T7.1: Política SL/TP client-side (2026-02-26):** `CLOSE_REASON_TTL` + `PaperExecutionEngine.check_ttl()` + `PaperRiskEngine(ttl_s=...)`. Tool `application/tools/run_paper_trade.py` (cicle open→monitor→close via HTTP). `compute_sl_tp()` + `check_sl_tp_triggered()`. Config: `PAPER_SL_PCT` (2%), `PAPER_TP_PCT` (4%), `PAPER_TTL_S` (3600s), `PAPER_POLL_S` (5s). 13 tests 0-network. Smoke: `python3 -m application.tools.run_paper_trade --symbol EURUSD --side long --collateral 100 --leverage 5 --ttl-s 60`.

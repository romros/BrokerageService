...
- **T5.16 (2026-02-24) Smoke LIVE Ostium:** `./scripts/run_ostium_live_smoke.sh --recreate` — run executat. Via nginx (8081): 504 Gateway Timeout (proxy_read_timeout 60s). Directe: `BASE_URL=http://127.0.0.1:8010` evita nginx; open pot trigar >120s (RPC chain). Quan PASS: open + close 200/202 amb `operation_id`; post-check via `GET /operations/{id}` fins `status=confirmed`; tx present.

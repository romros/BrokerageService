...
- ✅ **T9.07: RAW Dukascopy M1 BI5 immutable + sync incremental (2026-03):** Capa RAW canònica M1 BID BI5 per símbol/dia. Layout: `{DATAFILES_ROOT}/dukascopy_raw/m1_bi5_bid/{SYMBOL}/year=YYYY/month=MM/day=DD/BID_candles_min_1.bi5` + `manifest.json`; `watermark.json` per símbol. Escriptura atòmica (`.tmp` → rename), no-delete, immutable. Worker: `RawSyncWorker` (jobs a `jobs/raw_sync/{job_id}.json`, lock per un sol job), resumible. API: `POST /api/v1/data/raw/dukascopy/sync` (body: symbols, from_date, to_date, force) → job_id; `GET /api/v1/data/raw/dukascopy/status` (watermarks, running job); `GET /api/v1/data/raw/dukascopy/jobs/{job_id}`. Scheduler: `RAW_SYNC_ENABLED=1`, `RAW_SYNC_INTERVAL_MIN=60`, `RAW_SYNC_TAIL_DAYS=7`. Símbols: `SYMBOLS` (default EURUSD,XAUUSD). Verificar: `curl -s http://localhost:8081/api/v1/data/raw/dukascopy/status`; `curl -X POST .../raw/dukascopy/sync -d '{"symbols":["EURUSD"],"from_date":"2024-01-01","to_date":"2024-01-07"}'`. Docs: `docs/DUKASCOPY_RAW_STORE.md`.

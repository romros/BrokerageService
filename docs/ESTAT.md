...
- ✅ **T9.07/T9.08: Capa RAW Dukascopy M1 BI5 (2026-03):** Fitxers `.bi5` per dia, immutables, atòmics. Layout: `{DATAFILES_ROOT}/dukascopy_raw/m1_bi5_bid/{SYMBOL}/year=YYYY/month=MM/day=DD/BID_candles_min_1.bi5` + `manifest.json` + `watermark.json`. Worker intern `RawSyncWorker` (sense endpoints públics — és capa interna). Cron activat al `docker-compose.split.yml`: `RAW_SYNC_ENABLED=1`, `RAW_SYNC_INTERVAL_MIN=60`, `RAW_SYNC_TAIL_DAYS=7`. **EURUSD: ✅ COMPLET** (8339 fitxers, 2003-05-04 → 2026-03-02, zero dies sense fitxer). **XAUUSD: 🔄 en curs** (job `770a980976ba`, 2003-01-01 → 2026-03-03). Arquitectura: RAW → Parquet → DuckDB query (pas 2 pendent).

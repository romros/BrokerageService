...
- 🔄 **T9.15/T9.15.1: Gate SQ↔BS M1 parity (candles 1:1 a nivell d'API) (2026-03-04):** Compara candles M1 SQ (CSV) vs BS (GET /data/ohlcv). **No mira parquet** — només el que retorna el servei. **Policy:** `--policy intersection` (PASS si miss=0, mism=0; extra informatiu) per exports parcials; `--policy exact` (default, extra_in_bs=0) per exports complets. Script: `lab/datalayer/sq_bs_m1_parity_gate.py`. Wrapper: `./scripts/run_t915_sq_bs_m1_parity_gate.sh`. **Smoke:** `--policy intersection` → PASS; `--policy exact` → FAIL (extra_in_bs>0 amb export parcial). Verificar: `./scripts/run_t915_sq_bs_m1_parity_gate.sh --symbol EURUSD --from 2026-01-01 --to 2026-02-01 --policy intersection`. Artifacts: `lab/out/BS.T9.15_sq_bs_m1/{SYMBOL}/1m/{range}/` (gate_summary.json amb policy, months/YYYY-MM/, run.log).

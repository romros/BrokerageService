...
- ✅ **T7.2: LIVE/testnet smoke (2026-02-26):** Tool `application/tools/run_live_smoke_trade.py` — cicle mínim open→wait→close + close-idempotent. Mesura `open_ack_ms` + `close_ack_ms`. Artifact JSON estable: `datafiles/realtime_datalayer/artifacts/trading/latest_live_smoke_<SYMBOL>.json`. Ús: `python3 -m application.tools.run_live_smoke_trade --venue ostium --symbol EURUSD --side long --collateral 1.5 --leverage 2 --wait-s 10`.

...
- 🔄 **T9.15/T9.15.1/T9.15.2: Gate SQ↔BS M1 parity (candles 1:1 a nivell d'API) (2026-03-04):** Compara candles M1 SQ (CSV) vs BS (GET /data/ohlcv). **No mira parquet** — només el que retorna el servei. **Policy:** `--policy intersection` (PASS si miss=0, mism=0; extra informatiu) per exports parcials; `--policy exact` (default, extra_in_bs=0) per exports complets. Script: `lab/datalayer/sq_bs_m1_parity_gate.py`. Wrapper: `./scripts/run_t915_sq_bs_m1_parity_gate.sh`. **T9.15.2:** `sq_export_manifest.json` a artifacts; `--export-method`; script `run_t9152_export_sq_complete.sh` — prova `docker exec` (sqcli en marxa, com run_t842), fallback stop/run/start si "Another instance". **Smoke intersection:** t842 oracle → PASS. **Certificació exact pendent:** paritat_SQ_dukascopy_2024 dóna 83k mismatches (font SQ diferent de Parquet ticks); t842 (sqcli) dóna miss=0, mism=0 sobre intersecció però export parcial. Per exact PASS: export sqcli complet (1 mes) amb `./scripts/run_t9152_export_sq_complete.sh --from 2025-03-01 --to 2025-04-01` (aturar sqcli-docker abans) → gate amb `--policy exact --sq-input /mnt/volume-SQ/user/t915_export/`. **Quan exact falla per missing_in_bs:** executar T9.16 abans de rebuildar. Artifacts: `lab/out/BS.T9.15_sq_bs_m1/{SYMBOL}/1m/{range}/` (gate_summary.json, sq_export_manifest.json, months/, run.log).

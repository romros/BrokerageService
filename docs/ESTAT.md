...
- ✅ **T8.49: Gold Parity Suite (2026-03-01):** `lab/gold/` — màquina d'estats (DATA_ORACLE_READY → TRADES_PARITY_PASS), indicadors/execució certificats, expected_trades versionat. Cas `rsi35_exit60_m1_oracle`: RSI14 PRICE_CLOSE [1]<35, ExitAfterBars=60, same-bar reentry. **17/17 PASS.** Verificar: `./scripts/run_t849_gold_smoke.sh [--docker]`. Docs: `lab/gold/README.md`, `lab/gold/state_machine.md`.
- ✅ **T8.50: Lab cleanup (2026-03-01):** `scripts/lab_cleanup.py` + `run_t850_lab_cleanup.sh`. Flux: inventari → plan → dry-run → apply (git mv) cap a `lab/_archive/2026-03-01_lab_cleanup/`. KEEP: lab/gold/**, run_t84*.sh, out_compare camí canònic, docs principals. Arxiva: contract_*, artifacts, mt4like_*, ostium/out_ind. Verificar: `./scripts/run_t850_lab_cleanup.sh --dry-run` → `--apply`.
- ✅ **T9.07/T9.08: Capa RAW Dukascopy M1 BI5 (2026-03):** Fitxers `.bi5` per dia, immutables, atòmics. Layout: `{DATAFILES_ROOT}/dukascopy_raw/m1_bi5_bid/{SYMBOL}/year=YYYY/month=MM/day=DD/BID_candles_min_1.bi5` + `manifest.json` + `watermark.json`. Worker intern `RawSyncWorker` (sense endpoints públics — és capa interna). Cron activat al `docker-compose.split.yml`: `RAW_SYNC_ENABLED=1`, `RAW_SYNC_INTERVAL_MIN=60`, `RAW_SYNC_TAIL_DAYS=7`. **EURUSD: ✅ COMPLET** (8339 fitxers, 2003-05-04 → 2026-03-02, zero dies sense fitxer). **XAUUSD: ✅ COMPLET** (job `770a980976ba`). Arquitectura: RAW → Parquet → DuckDB query (pas 2 pendent).
- ✅ **T9.13: Parquet v2 ticks→M1 (2026-03-03):** Builder `application/tools/build_dukascopy_parquet_ticks.py`. Root v2: `{DATAFILES_ROOT}/historical_parquet_ticks_v1/{SYMBOL}/tf=1m/year=YYYY/month=MM/data.parquet`. Atomic write. Switch `DUKASCOPY_PARQUET_ACTIVE=ticks|legacy` (default=legacy). `DuckDBQueryService` resol root per env. **Pilot EURUSD 2025-03: 30.347 candles, 52 gaps (week+end normals), source=dukascopy_ticks_v1.** Smoke read OK (finestra reopen diumenge 2025-03-09). Legacy parquet intacte. 15/15 tests PASS. Artifacts: `lab/datalayer/artifacts/BS.T9.13/`.
- ✅ **T9.14: Escala Parquet v2 ticks + cutover (2026-03-04):** EURUSD 2003-01→2026-03 complet: 275 mesos, 8.5M candles, 0 errors. Retry 16 mesos fallats (timeout/503) OK; 2023-06 retry final OK. **Cutover fet:** `DUKASCOPY_PARQUET_ACTIVE=ticks` al `historical_datalayer` (docker-compose.split.yml). **Rollback:** `DUKASCOPY_PARQUET_ACTIVE=legacy`. Smoke windows OK (reopen 2025-03-09, 2004-02-03, 2024-11-12). Artifacts: `lab/out/artifacts/BS.T9.14/` (coverage_ticks_v2.json, smoke_windows.json). Rebuild mes: `docker exec historical-datalayer python3 application/tools/build_dukascopy_parquet_ticks.py --symbol EURUSD --from YYYY-MM-01 --to YYYY-MM-DD --out-root /datafiles/historical_parquet_ticks_v1 --raw-root /datafiles` (to exclusiu, p.ex. --to 2023-07-01 per juny).
- ✅ **T9.15/T9.15.1/T9.15.2: Gate SQ↔BS M1 parity (candles 1:1 a nivell d'API) (2026-03-04):** Compara candles M1 SQ (CSV) vs BS (GET /data/ohlcv). **No mira parquet** — només el que retorna el servei. **Policy:** `--policy intersection` (PASS si miss=0, mism=0; extra informatiu) per exports parcials; `--policy exact` (default, extra_in_bs=0) per exports complets. Script: `lab/datalayer/sq_bs_m1_parity_gate.py`. Wrapper: `./scripts/run_t915_sq_bs_m1_parity_gate.sh`. **Validació 7 barres:** Donem per vàlid que no podem obtenir-les de SQ; per totes les veles SQ tenim la mateixa (miss=0, mism=0); les 7 extra a BS són defecte SQ. **Policy intersection → PASS.** API filtra `market_closed` abans de retornar; Parquet dual (legacy + ticks v2); cutover `DUKASCOPY_PARQUET_ACTIVE=ticks`. Artifacts: `lab/out/BS.T9.15_sq_bs_m1/{SYMBOL}/1m/{range}/` (gate_summary.json, sq_export_manifest.json, months/, run.log).
- ✅ **T9.20: Eliminació filtre `is_market_open` de data_routes (2026-03-06):** El filtre `is_market_open` a `application/api/data_routes.py` eliminava barres DESPRÉS del DuckDB query (post-filtering), retornant pàgines amb < `PAGE_LIMIT` candles. El parity runner de SQRunner interpretava pàgines incompletes com a fi de dades i aturava la paginació prematurament (99k candles en lloc de 401k per EURUSD 2025). **Fix:** `candles_out = stitched["candles"]` (eliminar el list comprehension amb `is_market_open`). **Impacte BS:** les candles de cap de setmana ara són visibles via API (caps de setmana BS té candles sintètiques congelades — comportament esperat). **Impacte SQRunner:** paginació correcta, 401k candles en 81 pàgines. Paritat BS↔SQ EURUSD 2025: **3207/3220 (99.6%) trades PASS** (millor que 3210/3233 anterior per DST fix + ticks v2). 23 trades residuals: ~8 oracle xp/ep corrupte binari, ~8 t_close desfasat 1 barra (gap sessió puntual), ~5 RSI divergent per barra extra BS≠SQ. **Rollback:** restaurar filtre si necessari (nota: filtre trencava paginació). Rebuild canònic: `docker compose ... build historical_datalayer && up -d`.
- ✅ **T9.19: Decommission historical_parquet legacy (2026-03-04):** Ticks és l'únic camí actiu. `DUKASCOPY_PARQUET_ACTIVE=legacy` → error explícit. Arxivar legacy: `./scripts/run_t919_archive_legacy_parquet.sh` (servei aturat). Docs: `docs/_archive/2026-03-04_docs_cleanup/RESUM_T9.19_decommission_2026-03-04.md`.
- ✅ **T9.16: Dukascopy gap audit (RAW vs Parquet vs API) (2026-03-04):** `lab/datalayer/dukascopy_gap_audit.py` — diagnòstic quan T9.15 exact falla per `missing_in_bs`. Determina `root_cause` (raw_missing | raw_empty | raw_present_builder_gap | unknown), llista exacta `missing_minutes`, finestres contigües, estat raw per hora (exists/size/ticks_n), `suggested_rebuild.sh`. Wrapper: `./scripts/run_t916_gap_audit.sh`. Artifacts: `lab/out/BS.T9.16_gap_audit/{SYMBOL}/1m/{range}/`.
- 🔄 **T9.18: Certificar rang real SQ per EURUSD (contracte runner):** Certificar que BS API (source=dukascopy) retorna les mateixes candles que SQ **en el rang que SQ té** (inici SQ → 2026-01-28 exclusive). **Flux:** 1) Export SQCLI: `./scripts/run_t9152_export_sq_complete.sh --from <SQ_FROM> --to 2026-01-28`. 2) Gate exact: `./scripts/run_t915_sq_bs_m1_parity_gate.sh --symbol EURUSD --from <SQ_FROM> --to 2026-01-28 --policy exact --sq-input /mnt/volume-SQ/user/t915_export --export-method sqcli --resume`. **Una comanda:** `./scripts/run_t918_certify_sq_range.sh [--from 2023-06-15] [--to 2026-01-28]`. Si PASS: afegir línia **T9.18 certified** a ESTAT amb el rang; runner supported range = mateix. **Config:** `DUKASCOPY_CERTIFIED_FROM`, `DUKASCOPY_CERTIFIED_TO` (foundation/config/constants.py; opcional env al runner).
- ✅ **T9.12: Source= explicit routing (2026-03-03):** `?source=dukascopy|ostium` a `GET /ohlcv/{symbol}`. `SUPPORTED_SOURCES={"dukascopy","ostium"}`, `SOURCE_DEFAULT="dukascopy"`. `source=None` → fallback registry (`allowed_for_backtest`). `source` invàlid → 422. `GET /sources`, `GET /coverage/{symbol}?source=`. `get_ohlcv_backtest(source=...)` a `backtest_market_data.py`. Ostium_compat_registry = quality gate, no routing. **12/12 tests PASS.** Artifacts: `lab/datalayer/artifacts/BS.T9.12/`. Pendent: rebuild container per activar codi nou.
- 🔄 **T8.46: Signal Event Diff (oracle real) fins identificar bar 17è trade (2026-02-28):** signal_events_lab/sq, signal_event_diff, gap_report, first_divergence. **Fix aplicat:** round_decimals=None (MQ4 NormalizeDouble 6 decimals, no d1) → 14 matched, primer entry coincideix. **Divergències restants:** 3 MT4 unmatched (entry_ts 1770010200, 1770041760, 1770085800). Verificar: `./scripts/oneshot/run_t846_signal_event_diff.sh [--docker]`.
- 🔄 **T8.45: Execution Contract Grid (oracle real) fins 17/17 (2026-02-28):** Grid 2×2: entry open[i] vs open[i+1] × exit open vs close. RSI exact + round_decimals=1. **Resultat:** cap variant dona 17; totes 16 trades. v1/v2 (entry=open[i]): matched=13; v3/v4 (entry=open[i+1]): matched=0 (entry_ts desplaçat). **Conclusió:** el gap 16→17 no es resol amb execution contract. Artifacts: `execution_grid.csv`, `best_execution_variant.json`, `lab_trades_v1.csv`. Verificar: `./scripts/oneshot/run_t845_execution_grid.sh [--docker]`.
- 🔄 **T8.44: RSI SQ exact + getRounded half-up sweep (2026-03-01):** Candles oracle pass. RSI 1:1 RSICalculator.java (SMA seed + Wilder), getRounded amb half-up (34.5→35). Sweep decimals 0..4. Artifacts: `rounding_sweep.csv`, `best_rounding.json`, `mismatch_debug.csv`. **Resultat:** best_decimals=1, lab=16, target=17. mismatch_debug: ts 1770085800 rsi_raw=34.82, d0→35.0 (signal false), d1→34.8 (signal true). **execution_variant_suggestion:** entry open[i] vs open[i+1], exit open vs close. Verificar: `./scripts/oneshot/run_t844_rsi_sq_exact_rounding.sh [--docker]`.
- 🔄 **T8.43: Export Oracle REAL (.dat → CSV) + parity OFFLINE (2026-03-01):** Converteix `.dat` SQ a CSV oracle via sqcli one-off (stop sqcli → export → start) i executa harness sense BI5/xarxa. **Resultat real (2026-03-01):** candle_parity pass, lab_trades=16, mt4=17, matched=14. Mismatches: mt4_idx 5,10,17 (entry_ts 1770010200, 1770041760, 1770085800). **Comanda sqcli:** `docker compose run --rm sqcli sqcli -data action=export symbols=EURUSD_M1_dukas_M1_UTCMinus05 timeframe=M1 datefrom=2026.01.20 dateto=2026.02.03 outputdir=/home/squser/SQ/user/t842_oracle_export` (cal aturar sqcli-docker abans). **Comanda parity:** `./scripts/oneshot/run_t840_mt4_oracle_parity.sh --docker --no-api --lab-source oracle`. CSV: `user/t842_oracle_export/EURUSD_M1_dukas_M1_UTCMinus05-M1-No Session.csv` (15459 rows). Verificar: `./scripts/oneshot/run_t843_oracle_export_parity.sh [--docker]`.
- 🔄 **T8.42: SQ Oracle Candles + RSI exacta (2026-03-01):** Export candles oracle via sqcli (warmup 2026-01-20, eval 2026-02-01→03). Harness `--lab-source oracle`, RSI SQ-exact (1:1 RSICalculator.java), getRounded + rounding sweep (0..4 decimals). Scripts: `run_t842_sqcli_export_candles.sh`, `run_t842_sq_oracle_rsi_rounding.sh`. Artifacts T8.42: `t842_report.json`, `rounding_sweep.csv`, `lab_trades_round_*.csv`, `first_flip_window.csv`. Objectiu: 17/17 trades amb oracle candles. Verificar: `./scripts/oneshot/run_t842_sq_oracle_rsi_rounding.sh`.
- 🔄 **T8.40: MT4 Oracle Export + rerun (2026-03-01):** Tooling MQL4 `OracleExporterM1.mq4` (candles M1 + RSI buffer). Harness ampliat: parser candles (ts + SQ date), RSI parity, diagnòstic first_mismatch. Script `run_t840_mt4_oracle_parity.sh`. Artifacts T8.40: `t840_report.json`, candle/rsi/trade_mismatches. **Pendent:** export manual MT4 → mt4_oracle/ → rerun fins 17/17. Verificar: `./scripts/oneshot/run_t840_mt4_oracle_parity.sh --docker --no-api [--lab-source oracle]`.
- 🔄 **T8.39: Paritat M1 candles + 17 trades (2026-03-01):** Harness paritat "de baix a dalt" — candles M1 LAB (BI5/API), RSI Wilder, simulació trades (RSI[1]<35, exit 60 bars), comparació vs MT4. Script `run_t839_mt4_m1_parity.sh`. Artifacts: `artifacts/T8.39/EURUSD/1m/2026-02-01_2026-02-02/`. **Resultat inicial:** LAB 16 trades vs MT4 17 (matched 1, timestamps divergents). Verificar: `./scripts/oneshot/run_t839_mt4_m1_parity.sh --docker --no-api`.
- 🔄 **T8.38: MT4 Entry Gating Inference + Apply (2026-03-01):** Inferència cadència MT4 (median_bars_held=16) + grid gating (min_bars_after_exit, confirm_bars, max_entries_per_week). `entry_gating.py`, `entry_gating_infer.py`, `run_backtest.py --entry-gating-profile`. Script `run_t838_entry_gating_e2e.sh`. **Resultat baseline+gating:** best profile = no gating, mateix que baseline (29 trades, 14 matched). t836_best+gating pendent validar. Tests: `test_t838_entry_gating.py` (5 tests). Verificar: `./scripts/oneshot/run_t838_entry_gating_e2e.sh [--signal-def t836_best]`.
- 🔄 **T8.37: Apply t836_best signal_def (2026-03-01):** Estratègia `eurusd_ema200_rsi35_atr_d1` + export indicadors amb `signal_def=baseline|t836_best`. t836_best = RSI ema_gains sobre typical. Script `run_t837_t836_best_e2e.sh` executat. **Resultat:** baseline 14 matched (63.64% mt4) vs t836_best 10 matched (45.45% mt4). t836_best genera 91 trades (vs 29 baseline) i empitjora paritat. **NO graduar a policy.** NEXT_STEP: oracle indicadors MT4 o altres variants. Artifacts: `lab/runner/out_compare/artifacts/T8.37/`.
- 🔄 **T8.36: Signal Definition Sweep (2026-02-28):** `lab/runner/out_compare/signal_def_sweep.py` — grid 16 variants (RSI method, indexing, price source, rounding). Script `run_t836_signal_def_sweep.sh`. **Resultat:** best=ema_gains_bar_closed_typical_none, signal_true_at_t 18→22, NEXT_STEP=APPLY_RSI_SOURCE_TO_RUNNER.
- 🔄 **T8.35: MT4 Boundary & Concurrency Sanity Check (2026-02-28):** `lab/runner/out_compare/mt4_sanity_check.py` — inferència best_day_offset_h (0..23) i detecció overlaps. Script `run_t835_mt4_sanity_check.sh`. Artifacts: mt4_sanity_report.json, mt4_overlap_examples.csv. Tests: `test_mt4_sanity_check_smoke.py`. **Resultat:** best_day_offset_h=5, max_concurrency=1, NEXT_STEP=RSI_VARIANT_SWEEP.
- 🔄 **T8.34: D1 Series Shape Audit (2026-02-28):** `lab/runner/out_compare/d1_policy_audit.py` — policies Sunday bar (baseline, drop_sunday, merge_sunday_into_monday). Recomputa EMA/RSI/ATR + signal per cada policy. Script `run_t834_d1_policy_audit.sh`. Artifacts: d1_policy_audit.csv, d1_policy_audit.json, first_divergence_window_*.csv. Tests: `test_d1_policy_audit_smoke.py`. **Resultat:** best_policy=baseline, NEXT_STEP=RSI_VARIANT_SWEEP (cap policy millora el signal landscape).
- 🔄 **T8.33: Time Alignment Sweep (2026-02-28):** `lab/runner/out_compare/time_alignment_sweep.py` — sweep d'offsets [-12h..+12h] als timestamps MT4 per maximitzar matching i minimitzar CONTRACT_SHIFT/EXIT_CASCADE. Criteri best_offset: max matched, min shift+cascade, min signal_mismatch, min |offset|. Script `run_t833_time_alignment_sweep.sh`. Artifacts: time_alignment_sweep.csv, time_alignment_report.json, best_offset.txt, trade_diff_report_best_offset.json. Tests: `lab/runner/out_compare/tests/test_time_alignment_sweep_smoke.py`. **Resultat:** best_offset=0 (sense millora per offset; divergència no explicada per timezone).
- 🔄 **T8.32: Quick Parity Triage (2026-02-28):** `lab/runner/out_compare/quick_triage.py` — 4 micro-checks <20s per triar divergència (TIME_ALIGNMENT, DATA_MISSING, CONTRACT_SHIFT, INDICATOR_VARIANT). Usa trade_diff_report.json + indicators_LAB_full.csv; no recalcula. Script `run_t832_triage.sh`. Artifacts: `artifacts/T8.32/<strategy>/<symbol>/<tf>/<from_to>/` (triage_report.json, triage_*_check.json, window_around_divergence.csv, run.log). NEXT_STEP: DATA_REPAIR | TIME_ALIGNMENT_SWEEP | RSI_VARIANT_SWEEP | CONTRACT_SHIFT.
- 🔄 **T8.31: Trade Diff Analyzer (2026-03-01):** `lab/runner/out_compare/trade_diff_analyzer.py` — diagnòstic matched/unmatched MT4 vs LAB (best contract open_i). Classifica unmatched: DATA_MISSING, SIGNAL_MISMATCH, CONTRACT_SHIFT, EXIT_CASCADE. Identifica first divergence. Script `run_t831_trade_diff.sh`: export indicadors (--mt4-like) + analyzer. Artifacts: `indicators_LAB_full.csv`, `trade_diff_report.json`, `trade_diff_report.csv`. **Resultat:** entry_match_rate LAB 51.72%; 8 unmatched (2 SIGNAL_MISMATCH, 3 EXIT_CASCADE, 3 CONTRACT_SHIFT). First divergence: MT4 #1 2007-01-12, RSI LAB=46 vs MT4<35.
- 🔄 **T8.30: Parity MT4 — contracte senyal/entrada (2026-02-28):** `run_backtest.py`: `--entry-fill open_i|open_i1` i `--signal-contract mt4_baropen|v2` per explorar combinacions sense tocar l'estratègia. open_i=entrada a open[i] (MT4 On Bar Open), open_i1=entrada a open[i] amb senyal de i-1 (v2 original). Script `run_t830_contract_grid.sh`: backtest ×4 combinacions + compare_trades vs MT4, tria millor per entry_match_rate i n_trades proper a 22. Artifacts: `contract_grid_report.json`, `best_contract.txt`. Target: >60% entry_match_rate. Si cap millora → STOP "oracle necessari".
- 🔄 **T8.29A: MT4-like indicators auto + compare_trades (2026-02-28):** `application/data/indicators_mt4_like.py` — ema(seed_mode=sma|first), rsi_wilder, atr_wilder. run_backtest: --indicator-mode mt4_like, --ema-seed sma|first. Script `run_t829a_mt4like_compare.sh`: 2 backtests + compare_trades vs MT4 CSV, tria guanyador per entry_match_rate. Si no millora → STOP + report "next required". Artifacts: report_after_mt4like_sma.json, report_after_mt4like_first.json.
- 🔄 **T8.27: Reverse-engineer indicadors MT4/SQ (2026-02-28):** `docs/INDICATOR_PARITY_SPEC.md` — spec EMA (seed SMA), RSI Wilder, ATR Wilder. `lab/runner/indicators/mt4_like_indicators.py` — ema_mt4, rsi_wilder_mt4, atr_wilder_mt4. `export_indicators_csv.py --mt4-like`. Estratègia `eurusd_ema200_rsi35_atr_d1`: opció `mt4_like_indicators: true` al YAML. 7 tests `test_mt4_like_indicators.py`. Objectiu: millorar entry_match_rate vs MT4.
- 🔄 **T8.26: Repair 55 mesos buits Parquet via BI5 (2026-02-28):** `application/tools/repair_missing_months_bi5.py` — detecta mesos missing/num_rows=0/<threshold, rebaixa via `dukascopy_bi5.fetch_m1_month`, reescriu Parquet atòmic, rebuild coverage. Modus `--dry-run` (obligat abans) i `--fix`. Rang per defecte 2007-06→2011-12. `generate_parity_vs_sq_report.py` — regenera `parity_EURUSD_M1_vs_SQ.json` des de Parquet. Script: `./scripts/oneshot/run_t826_repair_bi5.sh` (dry-run→fix→parity report). Artifacts: `eurusd_m1_bad_months.json`, `repair_missing_months_report.json`, `parity_EURUSD_M1_vs_SQ.json`.
- ✅ **T8.25: Evidence pack post-BI5 (2026-02-28):** `missing_months_report.py` estès amb `empty_days_sample`, `rows_missing_estimate`, spot-checks BI5 (2–3 dies per mes buit). Spot-checks 2007-07/2008-03/2010-06: BI5 retorna HTTP 200 + 1440 rows; Parquet buit per 55 mesos explica delta -10.1%. Gate C: criteris PASS_BACKTEST explícits (corr≥0.90, dir_agree_filtered≥95%, window 1440m). Flat ratio: `flat_ratio_fx_interpretable` per FX. Script: `./scripts/run_t825_evidence_pack.sh`.
- ✅ **T8.24: Gate A PASS — resync EURUSD 2003-05→2006-12 via BI5 (2026-02-28):** Resync complet dels 44 mesos pre-2007 via `Bi5BackfillProvider`. **Fix aplicat:** `Bi5BackfillProvider` implementa `provider_name` + `max_range_minutes`; correccció invariant OHLC (`h=max(o,h,c)` / `l=min(o,l,c)`) per arrodoniment float del feed BI5. **Resultat:** 7,638,611 rows (era 5,756,530), coverage **2003-05→2026-02** (220 mesos), delta vs SQ **-10.1%** (era -32.3%). Gap residual = 55 mesos buits Dukascopy 2007-06→2011-12 (cap font pública disponible). **Gate B recheck:** invariants OK per rang 2004 BI5 (flat 28% = weekends normals). **Gate A: PASS.** Artifacts: `lab/runner/out_compare/parity_EURUSD_M1_after_bi5.json`.
- ✅ **T8.23: Dukascopy M1 pre-2007 via bi5 feed (2026-02-28):** Descobert endpoint binari natiu que SQ usa internament (`DataSourceDukascopy` → `DukascopyUtils`). URL: `https://datafeed.dukascopy.com/datafeed/{SYM}/{Y}/{M_0idx}/{D}/BID_candles_min_1.bi5`. Format .bi5: LZMA standalone + 24B/candle (ts_s uint32 BE, o/h/l/c uint32 BE ×10⁻⁵, vol float32 BE). Disponible EURUSD des de 2003-05-05 (HTTP 200, 1440 candles/dia). **Proves de descàrrega:** 2003-05-05→2003-05-07 = 4,320 candles OK. **Implementació:** `application/data/dukascopy_bi5.py` (parser + CLI), `infrastructure/venues/dukascopy/bi5_backfill_provider.py` (IBackfillProvider), `DukascopyBackfillProvider` fallback automàtic bi5 quan JSON retorna [] pre-2007. **15 tests 0-network** (`test_dukascopy_bi5.py`).
- 🟡 **T8.21: Indicator Parity Harness — seeding divergence diagnosada (2026-02-28):** `application/tools/export_indicators_csv.py` — exporta EMA200/RSI14/ATR14 LAB barra-a-barra (day_offset_h=5, warmup=300). `lab/runner/out_compare/compare_indicators.py` — compara indicadors MT4 vs LAB per ts. `lab/runner/mql4/IndicatorExporter.mq4` — EA MQL4 per exportar iMA/iRSI/iATR des de MT4. **Root cause confirmat per anàlisi matemàtic:** EMA200 LAB @ 2012-01 ≈ 1.330 vs MT4 ≈ 1.280 (**diff ~500 pips**) per gap Dukascopy 2007-06→2011-12 (55 mesos buits). EMA convergirà a <10 pips ~2013 (391 barres @ alpha=2/201). 6 trades MT4 2007-2011 **irreproduïbles** (cap dada). Trades 2013-2014: senyals ±4-6 dies desplaçats. **Fix pendent:** CSV MT4 via `IndicatorExporter.mq4` + seeding EMA extern. Report: `lab/runner/out_compare/indicator_seeding_report.json`.
- ✅ **T8.20: Intrabar exit modes sensitivity analysis (2026-02-28):** `run_backtest.py`: `resolve_sl_tp_hit(open, high, low, sl, tp, mode)` factoritzada + `--intrabar-mode {sl_first,tp_first,heuristic}`. Artifacts per mode no-default guardat a subdirectori `<mode>/`. **Resultat**: 3 modes produeixen **trades idèntics** (0 barres D1 amb dual SL+TP hit). Entry match-rate LAB vs MT4 = **50% en tots 3 modes** (delta=0%). **Conclusió**: la divergència LAB↔MT4 NO és per tick order intrabar. Causa residual: senyals EMA/RSI/ATR diferent entre LAB i MT4 per 4 trades missing. `recommendation=no_ticks_needed_for_d1_parity`. 11 tests 0-network (`test_intrabar_resolve.py`). Report: `lab/runner/out_compare/intrabar_sensitivity_report.json`.
- ✅ **T8.19: Fix expected_bar_count + Gate D recheck + compare_trades after_gate_b (2026-02-28):** `aggregation_report.py` corregit: expected calculat via calendari FX exacte (diumenge 21h→divendres 21h, exclou dissabte complet). Coverage ara ≤100% per tots els TF (H1=97.0%, H4=98.1%, D1=99.7%). Gate D recheck: backtest idèntic a T8.11 (18 trades, pnl=1.96%, entry_match_rate=50.0%) — estabilitat confirmada post-T8.16-T8.18. Report: `lab/runner/out_compare/report_after_gate_b.json`.
- ✅ **T8.18: Gate B (Aggregation M1→H1/H4/D1) PASS + Gate C recheck PASS (2026-02-28):** `application/tools/aggregation_report.py` — replica exacta de `aggregate_to_tf()` del runner LAB. Boundary D1 confirmat a **05:00 UTC** (day_offset_h=5). Rang 2020: H1=99.6%, H4=100.8%, D1=120.3% coverage, **0 invariants trencats**, flat=0%. Rang 2007 (amb gaps): invariants OK, gaps=weekends+mesos buits (esperats). Gate C recheck: EURUSD corr=0.956 dir_agree_filtered=99.0%, XAUUSD corr=0.959 dir_agree_filtered=96.9% — ambdós **PASS_BACKTEST**. `docs/DATA_PARITY_GATES.md` actualitzat: Gates B+C PASS.
...
**T9.10 watch:** job 9c9f42f95fa3 status=done → finalize executat. 9c9f42f95fa3 | days_done=6513 days_skipped=1 days_failed=123 | tmp_count=0 bi5_eurusd=6. Logs: `lab/datalayer/artifacts/BS.T9.10/watch.log`.

**T9.10.1 triage days_failed (no-delete, retry dirigit):** job 9c9f42f95fa3 days_failed=123. Extractor: `python3 scripts/t910_extract_failed_days.py --job-id 9c9f42f95fa3 --final-report lab/datalayer/artifacts/BS.T9.10/final_report.json` → `failed_days.csv` (123 dates, reason_code unknown/inferred_from_missing_raw), `failed_days_summary.json`, `gap_crosscheck_2026-02-27.json` (conclusion: gap_outside_job_range; job 2003-01-01→2021-03-03). Retry: `./scripts/run_t910_retry_failed_days.sh --failed-days lab/.../failed_days.csv [--max-days N] [--dry-run] [--resume]`; raw sospitós → `_archive/` (no delete). Artifacts: `lab/datalayer/artifacts/BS.T9.10/retry/run.log`, `retry_report.json`, `failed_after_retry.csv`. Smoke: dry-run --max-days 10 OK; retry real --max-days 2 → attempted=0 (API no retornà job_id), still_failed=2; cal gateway + historical_datalayer per retry efectiu.
